...
class TimerManager:
    ...
    def _register_timer_name_id(self, timer, timer_name, timer_id):
    ...
    def _cb_clear_timer(self, timer_name, timer_id):
        def clear_timer():
    ...
    def start_gametime_count_timer(self, timer_name, msg_queue, time_interval, max_ticks):
        ...
        timer = GametimeCountTimer(msg_queue, time_interval, max_ticks,
                                   down_count=True)
        # 添加退出回调，定时器退出时
        timer.add_exit_callback(self._cb_clear_timer(timer_name, timer_id))
        self._register_timer_name_id(timer, timer_name, timer_id)
